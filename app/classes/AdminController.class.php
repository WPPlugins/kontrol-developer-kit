...
class AdminController extends Lvc_PageController
{
	...
	protected function beforeAction()
	{
		...
		// Make this cache install unique
		$cache = get_option('kontrol_cache');
		if(empty($cache)) {
			$cache = Kontrol_Tools::generate_random_string(25);
			update_option('kontrol_cache', $cache);
		}
		$this->setLayoutVar('kontrol_cache', $cache);
