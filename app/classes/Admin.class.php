...
class KontrolAdmin
{
	...
	***********************/
	...
	{
		$find = new Kontrol_Tools();
		$files = $find->search_directory_recursive(get_template_directory(), 'kontrol-activate-'.APP_ID.'.php', '');
		if(isset($files[0]) && file_exists($files[0])) {
			include_once($files[0]);
			$dd0=$kontrolwp_wpkey;$sw1=$kontrolwp_skey;if(!$dd0){define('KONTROL_T',TRUE);}else{$ek2=!empty($sw1)?sha1($sw1.APP_ID.get_option('kontrol_cache',NULL)):NULL;$qo3=rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256,md5($ek2),base64_decode($dd0),MCRYPT_MODE_CBC,md5(md5($ek2))),"\0");($qo3==$sw1)?define('KONTROL_T',FALSE):define('KONTROL_T',TRUE);}
