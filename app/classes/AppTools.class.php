...
class Kontrol_Tools 
{
	...
	***********************/
	...
	/**********************
	* Recursive glob directory search
	* @author David Rugendyke
	* @since 1.0.4
	***********************/
	function search_directory_recursive($dir, $file = '', $ext = '', $found_files = array()) 
	{
		if(!empty($ext)) {
			$globFiles = glob("$dir/*.$ext");
		}
		if(!empty($file)) {
			$globFiles = glob("$dir/$file");
		...
		foreach ($globDirs as $dir) {
			$found_files = $this->search_directory_recursive($dir, $file, $ext, $found_files);
		}
